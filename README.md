...
- Terraform план для Cloud.ru запускается через `.github/workflows/terraform.yml` (секреты `CLOUDRU_TF_BACKEND_HCL`, `CLOUDRU_TFVARS`).
...
- Всегда запускать `predeploy_check.py` and `pytest tests/test_smoke.py`.
