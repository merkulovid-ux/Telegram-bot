...
**Основные технологии:**
- **Язык:** Python 3.11
- **Оркестрация:** Docker, Docker Compose (для локальной разработки), Yandex Compute Cloud (для продакшена)
...
Проект состоит из нескольких ключевых скриптов, работающих в Docker-контейнерах. Для продакшена, контейнеры будут развернуты на Yandex Compute Cloud VM.

- **`docker-compose.yml`**: Определяет два сервиса: `db` (PostgreSQL) и `app` (Python-приложение) для локальной разработки.
- **`Dockerfile`**: Собирает образ для сервиса `app`.
...
- **`ingest.py`**: Скрипт для начальной загрузки и обработки данных. Он выполняет следующие шаги:
    1.  Читает PDF-файлы из Yandex Object Storage.
    ...
    5.  Загружает данные в PostgreSQL.
- **`app.py`**: Основной скрипт, запускающий Telegram-бота.
- **`retriever.py`**: Модуль для взаимодействия с базой данных и Yandex API.
- **`llm.py`**: Модуль для взаимодействия с YandexGPT.
- **CI/CD с GitHub Actions**: Для автоматической сборки, тестирования и развертывания приложения на Yandex Compute Cloud VM.
- **Yandex Lockbox**: Для безопасного хранения секретов.
